...
class JacksonParser(logicalType: LogicalType) {
  ...
  private var rootRow: GenericRowData = _
   ...
   */
  private def makeRootConverter(logicalType: LogicalType): JsonParser => RowData = {
    logicalType match {
  ...
  private def makeStructRootConverter(rowType: RowType): JsonParser => RowData = {
    ...
    rootRow = new GenericRowData(names.size)
    (parser: JsonParser) => parseJsonToken[RowData](parser, rowType) {
      case START_OBJECT =>
        //convertObject(parser, names, fieldConverters)
        var i = 0
        while (i < names.size){
          rootRow.setField(i, null)
          i += 1
        }
        while (nextUntil(parser, JsonToken.END_OBJECT)) {
          names.get(parser.getCurrentName) match {
            case Some(index) =>
              rootRow.setField(index, fieldConverters(index).apply(parser))
            case None =>
              parser.skipChildren()
          }
        }
  ...
  def parse[T](
    record: T,
    createParser: (JsonFactory, T) => JsonParser): RowData = {
    try {
